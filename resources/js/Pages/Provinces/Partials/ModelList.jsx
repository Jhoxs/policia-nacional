...
const ModelList = ({ modelList }) => {
    ...
    const confirm = data => e => {
        router.delete(route('province.destroy', data), {
            onSuccess: () => {
                message.success('Se ha eliminado la provincia con éxito');
            }
        });
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <>
                    <Space size="middle">
                        {permissions.includes('province.edit') && (
                            <>
                                <Link href={route('province.edit', record.key)}>
                        ...
                        {permissions.includes('province.destroy') && (
                            <>
                                <Tooltip title={'Eliminar'}>
                                    <Popconfirm
                                        title='Eliminación de la Provincia'
                                        description={"¿Está seguro que desea eliminar la provincia " + record.display_name+ " ?"}
                                        okText='Si'
                                        cancelText='No'
    ...
    const handleSeach = (value, itemValue) => {
        ...
        router.visit(route('province.index', { value: value, key: iValue }), {
            preserveState: true,
            preserveScroll: true,
            method: 'get'
        })
